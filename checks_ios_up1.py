...
def evaluate_global_rule(rule, parsed_config):
    ...
    # AND requirements: every matcher must hit somewhere
    ...
    # OR requirements: at least one matcher must hit somewhere
    if required_any:
        any_hit_lines = []
    ...
    # Forbidden: none must match anywhere
...
def evaluate_scoped_rule(rule, parsed_config, strictness):
    ...
    # Missing block: resolve via strictness (existing behavior)
    if not blocks:
    ...
    for block in blocks:
        ...
        # AND requirements: all must match inside THIS block
        for matcher in required_all:
        ...
        # OR requirements: at least one must match inside THIS block
        if required_any:
            block_any_hits = []
            for matcher in required_any:
        ...
        # Forbidden: none must match inside THIS block
        for matcher in forbidden:
            hits = find_matching_lines(child_lines, matcher)
            for h in hits:
